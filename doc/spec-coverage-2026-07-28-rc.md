...
- The official conformance package is still alpha. The 2026 client suite keeps
  `json-schema-ref-no-deref` expected-failed because the alpha.8 mock server for
  that scenario still behaves like a stable-only server.
- The reverse Dart 2026 client -> TypeScript SDK beta server path currently
  covers discovery, `tools/list`, and `tools/call`. Broader reverse-path
  coverage should follow as the TypeScript SDK beta server surface stabilizes.
