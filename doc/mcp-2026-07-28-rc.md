...
`mcp_dart`'s `dev/2026-07-28` branch defaults to MCP `2026-07-28` draft/RC
preview negotiation. The default profile uses `server/discover` and stateless
request metadata with compatible peers, while retaining a stable
`2025-11-25` initialization fallback for older servers.
...
`McpClientOptions(protocol: McpProtocol.preview2026)`: it enables
...
Use the stable profile explicitly when a deployment must use only stable MCP
behavior:
...
`McpServerOptions()` also defaults to the preview profile. Servers advertise
and accept MCP `2026-07-28` draft/RC stateless requests alongside stable
versions:
...
Use `McpServerOptions(protocol: McpProtocol.stable)` to advertise only stable
...
| ------- | -------- | --------------- | --------------- |
| `McpProtocol.preview2026` | Yes | Tries `server/discover`, then falls back to `initialize` | Advertises stable and `2026-07-28` draft/RC protocol versions |
| `McpProtocol.stable` | No | Uses stable `initialize` | Advertises stable protocol versions |
...
branch's default preview profile. Select `McpProtocol.stable` explicitly when a
...
`McpProtocol.preview2026`. Select `McpProtocol.require2026` to forbid stable
fallback or `McpProtocol.stable` to retain stable-only behavior.
