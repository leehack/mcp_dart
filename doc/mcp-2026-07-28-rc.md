...
CI also runs it in the dedicated `Run MCP 2026-07-28 Interop`
workflow on relevant PRs, `dev/2026-07-28` pushes, a daily schedule, and
manual dispatch. That workflow also runs bidirectional Python SDK beta interop.
Keep the fixtures pinned to the draft behavior rather than one SDK's beta
behavior alone. The TypeScript fixture is currently pinned to published
`@modelcontextprotocol/client@2.0.0-beta.4` and
`@modelcontextprotocol/server@2.0.0-beta.4`, which expose the 2026 negotiation
...
Follow the complete
[`MCP 2026-07-28 day-0 release runbook`](mcp-2026-07-28-release-runbook.md)
once the official specification tag is available.
