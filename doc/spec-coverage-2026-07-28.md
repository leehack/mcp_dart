...
The alpha.11 frozen requirements pass every scored scenario: MCP 2025-11-25
server 30/30 and client 18/18; MCP 2026-07-28 server 37/37 and client 32/32.
The 2026 client score includes all 25 required authorization scenarios.
Extensions, pending checks, and post-release additions run as explicitly
unscored evidence rather than expected failures.
...
SDK 2.1.1 interop fixtures on relevant PRs, manual dispatch, and the daily
`main` schedule.
...
- Published TypeScript SDK 2.0.0 and Python SDK 2.1.1 include the post-#3002
...
- The official conformance package is still alpha. Alpha.11 freezes the
  requirement sets for each released revision, while unscored assertions and
  future requirement sets may continue to evolve. Every current scored client
  and server scenario passes, including the MCP 2026-07-28 network-`$ref`
  security canary.
...
- The Tasks extension is experimental and not an official MCP extension. Its
  alpha.11 scenarios run as unscored extension evidence and it remains absent
  from checked-in MCP 2026-07-28 cross-SDK interop. Do not describe it as
  stable, official, or cross-SDK verified.
