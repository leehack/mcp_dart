...
| [`oauth_client_example.dart`](oauth_client_example.dart) | Generic MCP 2025-11-25 initialization-era PKCE request, callback-state validation, token exchange, refresh, and storage building blocks | Uses `McpProtocol.legacy`; does not host a callback or target a real provider |
